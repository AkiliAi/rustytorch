// rustytorch_examples/src/new_reductions.rs
// Test rapide des nouvelles fonctionnalités de réduction

use rustytorch_tensor::Tensor;

pub fn run_new_reductions_demo() {
    ...
    println!(
        "Cumprod result: {:?}",
        cumprod_result.storage().to_vec_f64()
    );
    ...
    println!(
        "Frobenius norm: {:?}",
        frob_norm.storage().get_f64(0).unwrap()
    );
