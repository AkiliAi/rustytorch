...
impl Tensor {
    ...
    pub fn apply_unary_op<F32Op, F64Op>(
        &self,
        f32_op: F32Op,
        f64_op: F64Op,
    ...
    pub fn apply_binary_op<F32Op, F64Op>(
        &self,
        other: &Self,
        f32_op: F32Op,
        f64_op: F64Op,
    ...
    pub fn atan(&self) -> Result<Self, TensorError> {
        self.apply_unary_op(|x| x.atan(), |x| x.atan())
    }
        ...
        if self.ndim() == 1 {
            match self.storage.as_ref() {
                StorageType::F32(data) => {
                    ...
                    let mut exp_values: Vec<f32> =
                        data.iter().map(|&x| (x - max_val).exp()).collect();
    ...
    pub fn sigmoid_backward(&self, grad_output: &Self) -> Result<Self, TensorError> {
        ...
        sigmoid_result.apply_binary_op(
            grad_output,
    ...
    pub fn rad2deg(&self) -> Result<Self, TensorError> {
        ...
        self.apply_unary_op(|x| x * rad_to_deg_f32, |x| x * rad_to_deg_f64)
    }
