...
pub trait NumericOps<Rhs = Self> {
    ...
    fn neg(self) -> Result<Self::Output> where Self: Sized;
    
    /// Absolute value
    fn abs(self) -> Result<Self::Output> where Self: Sized;
...
pub trait Reshapable {
    ...
    fn reshape(&self, shape: &[usize]) -> Result<Self> where Self: Sized;
    
    /// Flatten tensor to 1D
    fn flatten(&self) -> Result<Self> where Self: Sized;
    
    /// Transpose two dimensions
    fn transpose(&self, dim0: usize, dim1: usize) -> Result<Self> where Self: Sized;
    
    /// Permute dimensions according to the given order
    fn permute(&self, dims: &[usize]) -> Result<Self> where Self: Sized;
    
    /// Remove dimensions of size 1
    fn squeeze(&self, dim: Option<usize>) -> Result<Self> where Self: Sized;
    
    /// Add a dimension of size 1
    fn unsqueeze(&self, dim: usize) -> Result<Self> where Self: Sized;
    
    /// View tensor with new shape (without copying data)
    fn view(&self, shape: &[isize]) -> Result<Self> where Self: Sized;
...
pub trait Indexable {
    ...
    fn slice(&self, ranges: &[Range<usize>]) -> Result<Self> where Self: Sized;
    
    /// Advanced indexing with tensor indices
    fn index(&self, indices: &Self::Index) -> Result<Self> where Self: Sized;
    
    /// Masked selection
    fn masked_select(&self, mask: &Self) -> Result<Self> where Self: Sized;
    
    /// Gather values along an axis
    fn gather(&self, dim: usize, indices: &Self::Index) -> Result<Self> where Self: Sized;
    
    /// Scatter values along an axis
    fn scatter(&mut self, dim: usize, indices: &Self::Index, values: &Self) -> Result<()> where Self: Sized;
}
...
pub trait Broadcasting {
    ...
    fn broadcast_tensors(tensors: &[&Self]) -> Result<Vec<Self>> where Self: Sized;
}
...
pub trait Differentiable {
    ...
    fn backward(&self, gradient: Option<Self::Gradient>, retain_graph: bool, create_graph: bool) -> Result<()>;
    ...
    fn register_hook<F>(&mut self, hook: F) where F: Fn(&Self::Gradient) -> Self::Gradient + 'static;
}
...
pub trait Serializable {
    ...
    /// Load from a reader
    fn load_from<R: std::io::Read>(reader: &mut R) -> Result<Self> where Self: Sized;
    
    /// Export to numpy-compatible format
    fn to_numpy(&self) -> Result<Vec<u8>>;
    
    /// Import from numpy-compatible format
    fn from_numpy(data: &[u8]) -> Result<Self> where Self: Sized;
}